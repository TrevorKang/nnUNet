...
DEFAULT_SOURCE = REPO_ROOT / "data" / "MRI"
...
def collect_cases(source: Path, splits: Iterable[str]) -> list[MRICase]:
    cases = []
    seen_case_ids = set()
    for split in splits:
...
def convert_cases(cases: Iterable[MRICase], images_dir: Path, labels_dir: Path) -> int:
    count = 0
    for case in cases:
...
def make_dataset(
    ...
    generate_dataset_json(
        ...
        description="MRI ovary segmentation converted from PNG images and binary PNG masks.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert data/MRI to nnU-Net v2 format. Train and valid are merged into imagesTr/labelsTr; "
            "test is kept separate in imagesTs/labelsTs."
        )
    )
    parser.add_argument("--source", type=Path, default=DEFAULT_SOURCE, help=f"MRI source folder. Default: {DEFAULT_SOURCE}")
    parser.add_argument(
        "--nnunet-raw",
        type=Path,
    ...
    parser.add_argument("--dataset-id", type=int, default=231, help="nnU-Net dataset id. Default: 231")
    parser.add_argument("--dataset-name", default="MRI", help="nnU-Net dataset name. Default: MRI")
...
if __name__ == "__main__":
    ...
    print(f"Converted MRI dataset to: {output}")
